...
def query(input_data, price_max):
    inputs = (np.asfarray(input_data.values).reshape(input_nodes) / price_max * 0.99) + 0.01
...
if __name__ == '__main__':
    ...
    sql_cmd = 'select open, close, high, low from stock_daily order by trade_date asc limit %d' % stock_max_row
    df = pd.read_sql(sql=sql_cmd, con=db)

    sql_cmd_date = 'select trade_date from stock_daily order by trade_date asc limit %d' % stock_max_row
    df_date = pd.read_sql(sql=sql_cmd_date, con=db)
    ...
    target_outputs = []
    predicted_outputs = []
    trade_dates = np.asanyarray(df_date[train_times + input_days - 1:train_times + input_days + test_days -1]).reshape(test_days)
    ...
    for i in range(train_times, train_times + test_days):
        ...
        predicted_outputs.append(query(df[i:input_days + i], high_max))
