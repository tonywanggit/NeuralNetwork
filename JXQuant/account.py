    ...
    def __init__(self, state_dt):
        try:
            ...
            if len(stock_pool_records) > 0:
                ...
                self.stock_map_buyprice = {x[0]: float(x[1]) for x in stock_pool_records}
                self.stock_map_hold_volumn = {x[0]: int(x[2]) for x in stock_pool_records}
                self.stock_map_hold_days = {x[0]: int(x[3]) for x in stock_pool_records}
