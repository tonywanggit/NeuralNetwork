...
def buy(stock_code, opdate, buy_money):
    ...
    if deal_buy.cur_money_rest + 1 >= buy_money:
        sql_buy = "select close from stock_daily a where a.trade_date = '%s' and a.ts_code = '%s'" % (opdate, stock_code)
        cursor.execute(sql_buy)
        done_set_buy = cursor.fetchall()
        if len(done_set_buy) == 0:
            return -1

        buy_price = float(done_set_buy[0][0])
        if buy_price >= 195:
            return 0
        ...
        if stock_code in deal_buy.stock_all:
            new_buy_price = (deal_buy.stock_map_buyprice[stock_code] * deal_buy.stock_map_hold_volumn[
                stock_code] + vol * buy_price) / (deal_buy.stock_map_hold_volumn[stock_code] + vol)
            new_vol = deal_buy.stock_map_hold_volumn[stock_code] + vol
...
def sell(stock_code, opdate, predict):
    ...
    init_price = deal.stock_map_buyprice[stock_code]
    hold_vol = deal.stock_map_hold_volumn[stock_code]
    hold_days = deal.stock_map_hold_days[stock_code]
    sql_sell_select = "select close from stock_daily a where a.trade_date = '%s' and a.ts_code = '%s'" % (
    opdate, stock_code)
    cursor.execute(sql_sell_select)
    done_set_sell_select = cursor.fetchall()
    if len(done_set_sell_select) == 0:
        return -1
    sell_price = float(done_set_sell_select[0][0])
